// ============================================================
// WeatherNext Service Worker — Perak (霹雳)
// MIGRATED to the full Raub/Cameron microclimate architecture (was the older
// 1.0.155 forecast-only build, no boot screen — one is added here). NOTE: this
// is the first NON-Pahang build — the AI prompt and branding say Perak, and the
// lowland framing still applies (these farms are lowland Perak). Carries:
// microclimate disease-risk engine (6-disease + Phase-2 tiers), fog engine,
// 29-crop list, coordinate-aware terrain note, broadcast GPS sort, Open-Meteo
// rate-limit throttling + retry, storm-confidence wording, AI-greeting
// crop-owner fix, REAL model-run freshness header, AI prompt with per-farm
// coordinates (keeps Perak name). Boot screen sky-blue (#82c6f0) to match the
// orchard/creek icon. Identity: namespace weathernextforperak, appId
// wnext-ag-v41-weathernextforperak, name 胡瑞良, 4 seed farms (Chenderiang /
// Sungkai / Diamond Creeks / Tanjung Malim, c_pk- IDs preserved), seed version
...
// with the refined Raub version. Perak identity is UNCHANGED —
// weathernextforperak namespace, appId wnext-ag-v41-weathernextforperak, name
// 胡瑞良, 4 seed farms (Chenderiang / Sungkai / Diamond Creeks / Tanjung Malim,
// c_pk- IDs), seed version pk-arch1, the Perak (first non-Pahang) AI prompt +
// branding, and the build's GPS sort are all preserved exactly. Only the
// broadcast text logic was swapped (function body verified against the pre-fix
// baseline, zero build-specific drift). Fixes carried over:
...
const CACHE_VERSION = 'wnext-weathernextforperak-202606060045';
